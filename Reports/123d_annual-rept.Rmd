...
Figure 1. Removal reaches sampled during 2021: Tusher Reach (RM `r s_summary_rch$LGR$rmi_max` - `r s_summary_rch$LGR$rmi_end_bel_min`) and Lower Colorado Reach (RM `r s_summary_rch$LCO$rmi_start_bel_max` and`r s_summary_rch$LCO$rmi_end_bel_min`).
...
<center>
```{r fig.cap="A photograph showing UDWR fisheries technician Carlos Montanez with 14 tagged walleye on the deck of an electrofishing boat. All fish were later frozen and transported to Fort Collins, CO for use in an energetics study. A scale and measuring board are also visible."}
knitr::include_graphics(path = paste0(assets_path, gr_we),
                        dpi = 75)
```
</center>  

UDWR fisheries technician Carlos Montanez with 14 tagged walleye on the deck of an electrofishing boat. All fish were later frozen and transported to Fort Collins, CO for use in an energetics study. 
...
<center>
```{r fig.cap="A photograph showing a researcher on the front of an electrofishing boat, holding a dip net. The black shist and sandstone canyons of Westwater are visible in the background."}
